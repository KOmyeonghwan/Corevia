...
public interface UsersRepository extends JpaRepository<User, Integer> {
    ...
    // 특정 부서의 부장 조회
    List<User> findByDepartment_IdAndPosition_Id(Integer departmentId, Integer positionId);

    // 특정 부서에 부장이 존재하는지
    boolean existsByDepartment_IdAndPosition_Id(Integer departmentId, Integer positionId);

    @Query("""
            SELECT u FROM User u
            ORDER BY
            CASE
                WHEN u.position.id = 1 THEN 0
                WHEN u.position.id = 2 THEN 1
                ELSE 2
            END,
            u.jobcode ASC
            """)
    List<User> findAllOrderByCeoFirst();

    @Query("""
            SELECT u FROM User u
            ORDER BY
            CASE
                WHEN u.position.id = 2 THEN 0
                ELSE 1
            END,
            u.jobcode ASC
            """)
    List<User> findAllOrderByManagerFirst();
    ...
    @Query("""
            ...
            u.jobcode ASC
            """)
    List<User> findByDepartmentOrderByManagerFirst(@Param("departmentId") Integer departmentId);
